...
from src.billing.models import Price
...
class ExternalReview(Activity):
    ...
    def get_final_urls(self):
        ...
        return super().get_final_urls() + [
            reverse('core:checklist_review', kwargs={'submission_form_pk': sf, 'blueprint_pk': blueprint_id})
            for sf in self.workflow.data.submission.forms.values_list('pk', flat=True)
        ]

    def get_afterlife_url(self):
        c = self.workflow.data
        return '{0}#checklist_{1}_review_form'.format(reverse('core:view_submission', kwargs={'submission_pk': c.submission.pk}), c.pk)
    ...
    def receive_token(self, *args, **kwargs):
        ...
        if c.status == 'new':
            ...
            url = reverse('tasksv:do_task', kwargs={'task_pk': token.task.pk})
...
class ExternalReviewReview(Activity):
    ...
    def get_url(self):
        checklist = self.workflow.data
        submission_form_id = checklist.submission.current_submission_form_id
        return reverse('core:show_checklist_review', kwargs={'submission_form_pk': submission_form_id, 'checklist_pk': checklist.pk})
