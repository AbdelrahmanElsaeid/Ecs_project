...
from src.workflow.exceptions import TokenRequired, WorkflowError
from src import workflow
from src.workflow.models import Token
from src.workflow.tests.models import Foo, FooReview
...
class FlowTest(WorkflowTestCase):
    ...
    def test_trail(self):
        ...
        self.assertEqual(set(b_token.trail.all()), {a_token})
        self.assertEqual(set(c_token.trail.all()), {a_token})
        ...
        self.assertEqual(b_token.activity_trail, {a_token})
        self.assertEqual(c_token.activity_trail, {a_token})
        self.assertEqual(e_token.activity_trail, {b_token, c_token})
