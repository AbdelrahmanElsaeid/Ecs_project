...
def register(model, autostart_if=None):
    ...
    from src.workflow.descriptors import WorkflowDescriptor
    model.workflow = WorkflowDescriptor()
    
def autodiscover():
    import src.workflow.patterns
...
def _post_save(sender, **kwargs):
    ...
    if not kwargs.get('raw') and autostart_if(obj, kwargs['created']):
        from src.workflow.models import Workflow, Graph
        from django.contrib.contenttypes.models import ContentType
        from django.db.models import Model
...
default_app_config = 'src.workflow.apps.WorkflowAppConfig'