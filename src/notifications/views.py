...
from src.utils.viewutils import render_html, redirect_to_next_url
from src.docstash.decorators import with_docstash
from src.docstash.models import DocStash
from src.core.forms.layout import get_notification_form_tabs
...
from src.notifications.forms import (
    NotificationAnswerForm, RejectableNotificationAnswerForm,
    AmendmentAnswerForm,
)
from src.documents.views import handle_download, upload_document, delete_document
from src.users.utils import user_group_required
from src.tasksv.utils import task_required, with_task_management
...
def open_notifications(request):
    ...
    stashed_notifications = DocStash.objects.filter(
        owner=request.user, group='notifications:create_notification',
...
def view_notification(request, notification_pk=None):
    ...
    print(f" i in view not func ---------- {notification}")
...
def create_diff_notification(request, submission_form_pk=None, notification_type_pk=None):
    ...
    docstash = DocStash.objects.create(
        group='notifications:create_notification',
...
@with_docstash(group='notifications:create_notification')
