...
from src.users.utils import sudo, get_current_user
...
def get_reviewing_parties(sf, active=None):
    ...
    with sudo():
        tasks = Task.objects.for_submission(sf.submission).filter(assigned_to__isnull=False, deleted_at=None).exclude(task_type__workflow_node__uid__in=('resubmission', 'b2_resubmission')).order_by('created_at').select_related('task_type').distinct()
        if active:
            tasks = tasks.open()
        tasks = list(tasks)
    for task in tasks:
