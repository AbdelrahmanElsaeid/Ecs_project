...
from src.checklists.models import ChecklistBlueprint, Checklist
...
from src.core.forms.review import CategorizationForm, BiasedBoardMemberForm
from src.core.forms.layout import SUBMISSION_FORM_TABS
from src.votes.forms import VoteReviewForm, VotePreparationForm, B2VotePreparationForm
from src.core.forms.utils import submission_form_to_dict
from src.checklists.forms import ChecklistAnswerFormSet
from src.notifications.models import (
    Notification, NotificationType, CenterCloseNotification,
    AmendmentNotification, SafetyNotification, NotificationAnswer,
)

from src.core.workflow import ChecklistReview

from src.core.signals import on_study_submit, on_presenter_change, on_susar_presenter_change
...
from src.users.utils import sudo, user_flag_required, get_user
from src.tasksv.models import Task
from src.tasksv.utils import get_obj_tasks, task_required, with_task_management

from src.documents.views import upload_document, delete_document
...
def copy_submission_form(request, submission_form_pk=None, notification_type_pk=None, delete=False):
    ...
    if delete:
        docstash = DocStash.objects.create(
            group='core:create_submission_form',
            owner=request.user,
        )
        created = True
    else:
        for docstash in DocStash.objects.filter(group='notifications:create_notification', owner=request.user):
    ...
    return redirect('core:create_submission_form',
        docstash_key=docstash.key)
...
def copy_latest_submission_form(request, submission_pk=None, **kwargs):
    ...
    return redirect('core:copy_submission_form', **kwargs)


def view_submission(request, submission_pk=None):
    submission = get_object_or_404(Submission, pk=submission_pk)
    return redirect('core:readonly_submission_form', submission_form_pk=submission.current_submission_form.pk)
...
def readonly_submission_form(request, submission_form_pk=None, submission_form=None, extra_context=None, template='submissions/readonly_form.html', checklist_overwrite=None):
    ...
    for d in DocStash.objects.filter(owner=request.user, group='notifications:create_notification'):
        if not d.POST:
            continue
...
def paper_submission_review(request, submission_pk=None):
    ...
    if not task.assigned_to == request.user:
        ...
        return redirect('core:paper_submission_review', submission_pk=submission_pk)
    return readonly_submission_form(request, submission_form=submission.current_submission_form)
...
def biased_board_members(request, submission_pk=None):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        return redirect('core:biased_board_members',
...
def checklist_review(request, submission_form_pk=None, blueprint_pk=None):
    ...
    if request.method == 'GET' and not submission_form.is_current:
        return redirect('core:checklist_review', submission_form_pk=submission_form.submission.current_submission_form.pk, blueprint_pk=blueprint_pk)
    blueprint = get_object_or_404(ChecklistBlueprint, pk=blueprint_pk)
...
@with_docstash(group='core:create_submission_form')
...
def delete_document_from_submission(request):
    ...
    return redirect('core:upload_document_for_submission',
        docstash_key=request.docstash.key)

@with_docstash()
def create_submission_form(request):
    docstash_key = request.docstash.key  # Get the docstash.pk value
...
def change_submission_presenter(request, submission_pk=None):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        if request.user == previous_presenter:
            ...
            return redirect('dashboard:view_submission', submission_pk=submission.pk)
...
def delete_docstash_entry(request):
    request.docstash.delete()
    return redirect_to_next_url(request, reverse('dashboard:view_dashboard'))
...
def submission_list(request, submissions, stashed_submission_forms=None, template='submissions/list.html', keyword=None, filter_form=SubmissionFilterForm, filtername='submission_filter', extra_context=None, title=None):
    ...
    try:
        submissions = paginator.page(filterform.cleaned_data['page'])
...
def xls_export(request):
    from src.core.tasks import xls_export
    ...
    return redirect('core:all_submissions')
...
def all_submissions(request):
    ...
    print(f"sublast is in func ----->   {submissions}")
...
def my_submissions(request):
    ...
    stashed = (DocStash.objects
        .filter(group='core:create_submission_form',
...
def catalog(request, year=None):
    with sudo():
        ...
        if year is None:
            ...
            return redirect('core:catalog_year', year=year)
        else:
            year = int(year)
...
def grant_temporary_access(request, submission_pk=None):
    ...
    if form.is_valid():
        ...
        temp_auth.save()
    return redirect(reverse('core:view_submission', kwargs={'submission_pk': submission_pk}) + '#involved_parties_tab')
