...
from src.users.utils import get_or_create_user
...
VALID_SUBMISSION_FORM_DATA = {
    ...
    'substance_preexisting_clinical_tries': ['2'], 'substance_p_c_t_phase': ['III'], 'subject_males': ['2'], 'investigator-0-phone': [''], 
    ...
    'subject_females_childbearing': ['0'], 'investigator-0-organisation': ['orga'], 'sponsor_contact_first_name': ['first'],
    ...
    'german_primary_hypothesis': ['bla bla bla'], 'substance_p_c_t_countries': ['AT','DE','CH'], 
...
class SubmissionViewsTestCase(LoginTestCase):
    ...
    def get_docstash_url(self):
        url = reverse('src.core.views.submissions.create_submission_form')
    ...
    def test_submission_pdf(self):
        ...
        submission_form = create_submission_form()
        response = self.client.get(reverse('src.core.views.submissions.submission_form_pdf', kwargs={'submission_form_pk': submission_form.pk}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_document_download(self):
        ...
        response = self.client.get(reverse('src.core.views.submissions.download_document', kwargs={'submission_form_pk': submission_form.pk, 'document_pk': document_pk}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response['Content-Type'], 'application/pdf')
        self.assertEqual(next(response.streaming_content)[:5], b'%PDF-')
    ...
    def test_submission_form_search(self):
        ...
        url = reverse('src.core.views.submissions.all_submissions')
    ...
    def test_submission_form_copy(self):
        ...
        response = self.client.get(reverse('src.core.views.submissions.copy_latest_submission_form', kwargs={'submission_pk': submission_form.submission.pk}))
        self.assertEqual(response.status_code, 302)
        url = reverse('src.core.views.submissions.copy_submission_form', kwargs={'submission_form_pk': submission_form.pk})
    ...
    def test_initial_review(self):
        ...
        response = self.client.post(reverse('src.tasks.views.accept_task', kwargs={'task_pk': task.pk}))
        self.assertEqual(response.status_code, 302)
        task = refetch()
