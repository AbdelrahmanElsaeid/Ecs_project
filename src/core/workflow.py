...
from src.core.models.constants import SUBMISSION_LANE_RETROSPECTIVE_THESIS
from src.core.signals import on_initial_review, on_categorization
from src.checklists.models import ChecklistBlueprint, Checklist, ChecklistAnswer
from src.checklists.utils import get_checklist_answer
...
class InitialReview(Activity):
    ...
    def get_url(self):
        return reverse('core:initial_review', kwargs={'submission_pk': self.workflow.data_id})
...
class Resubmission(Activity):
    ...
    def get_url(self):
        return reverse('core:copy_latest_submission_form', kwargs={'submission_pk': self.workflow.data_id})
...
class B2ResubmissionReview(Activity):
    ...
    def get_url(self):
        return reverse('core:b2_vote_preparation', kwargs={'submission_form_pk': self.workflow.data.newest_submission_form.pk})
...
class Categorization(Activity):
    ...
    def get_url(self):
        return reverse('core:categorization',
...
def unlock_categorization(sender, instance, **kwargs):
    if hasattr(instance, 'workflow_lane') and callable(getattr(instance, 'workflow_lane', None)):
        instance.workflow_lane.unlock(Categorization)
...
class CategorizationReview(Activity):
    ...
    def get_url(self):
        return reverse('core:categorization_review',
            kwargs={'submission_pk': self.workflow.data_id})
...
class PaperSubmissionReview(Activity):
    ...
    def get_url(self):
        return reverse('core:paper_submission_review', kwargs={'submission_pk': self.workflow.data_id})
...
class ChecklistReview(Activity):
    ...
    def get_url(self):
        ...
        return reverse('core:checklist_review', kwargs={'submission_form_pk': submission_form_id, 'blueprint_pk': blueprint_id})
...
class VotePreparation(Activity):
    ...
    def get_url(self):
        return reverse('core:vote_preparation', kwargs={'submission_form_pk': self.workflow.data.current_submission_form_id})
