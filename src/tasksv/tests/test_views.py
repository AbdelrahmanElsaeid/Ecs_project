...
class ViewTestCase(LoginTestCase, WorkflowTestCase):
    ...
    def test_common_workflow(self):
        ...
        # show the task list
        response = self.client.get(reverse('tasks.views.my_tasks'))
        self.assertEqual(response.status_code, 200)
        self.assertTrue(task in response.context['open_tasks'])
        
        # change the task list filter
        response = self.client.post(reverse('tasks.views.my_tasks'), {'proxy': False, 'mine': True, 'open': True, 'amg': False, 'mpg': False, 'other': True})
        self.assertEqual(response.status_code, 200)
        self.assertTrue(task in response.context['open_tasks'])
        
        # accept the task
        response = self.client.post(reverse('tasks.views.accept_task', kwargs={'task_pk': task.pk}))
        task = refetch()
