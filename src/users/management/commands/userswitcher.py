...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('-i', action='store_true', dest='activate_internal', help='add is_internal users to the userswitcher')
        parser.add_argument('-I', action='store_false', dest='activate_internal', help='remove is_internal users from the userswitcher')
        parser.add_argument('-t', action='store_true', dest='activate_testusers', help='add testusers to the userswitcher')
        parser.add_argument('-T', action='store_false', dest='activate_testusers', help='remove testusers from the userswitcher')
        parser.add_argument('-m', action='store_true', dest='activate_board', help='add board member users to the userswitcher')
        parser.add_argument('-M', action='store_false', dest='activate_board', help='remove board member users from the userswitcher')
        parser.add_argument('-r', action='store_true', dest='remove_user', help='remove users from the userswitcher')
    ...
    def handle(self, *args, **options):
        ...
        internal_users = User.objects.filter(profile__is_internal=True).exclude(
            profile__is_testuser=True)
        ...
        def _toggle(user, activate):
            if verbosity >= 2:
                print(f'  {"+" if activate else "-"} {user.email}')
            if activate:
