...
def html2text(htmltext):
    text = unescape(strip_tags(htmltext))
    text = '\n\n'.join(re.split(r'\s*\n\s*\n\s*', text))
...
def create_mail(subject, message, from_email, recipient, message_html=None,
    ...
    if from_email == settings.DEFAULT_FROM_EMAIL:
        try:
            ...
            default_contact = advanced_settings.default_contact
            headers.update({'Reply-To': default_contact})
        except AdvancedSettings.DoesNotExist:
            # Handle the case where AdvancedSettings with pk=1 does not exist
            pass

        # if empty, set Auto-Submitted for mails from DEFAULT_FROM_EMAIL
