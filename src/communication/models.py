...
class Thread(models.Model):
    ...
    sender = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='outgoing_threads', on_delete=models.CASCADE)
    receiver = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='incoming_threads', on_delete=models.CASCADE)
    timestamp = models.DateTimeField(auto_now_add=True)
    last_message = models.OneToOneField('Message', null=True,blank=True, related_name='head', on_delete=models.CASCADE)
    related_thread = models.ForeignKey('self',null=True,blank=True, on_delete=models.CASCADE)
...
class Message(models.Model):
    ...
    reply_receiver = models.ForeignKey(User, null=True, related_name='reply_receiver_for_messages', on_delete=models.CASCADE)
    timestamp = models.DateTimeField(auto_now_add=True)
    ...
    smtp_delivery_state = models.CharField(max_length=7, choices=DELIVERY_STATES, default='new', db_index=True)
    in_reply_to = models.ForeignKey('self', related_name='is_replied_in', null=True,blank=True, default=None, on_delete=models.CASCADE)
    creator = models.CharField(max_length=14, choices=CREATOR_CHOICES, default='human')
    ...
    def smtp_subject(self):
        ...
        subject = _('[ECS{ec_number}] {subject}.').format(ec_number=ec_number, subject=self.thread.subject)
        return subject
            ...
            if self.creator in ("human", "auto-self"):
                ...
                if self.incoming_msgid:
                    headers.update({
                        ...
                        'References': " ".join((self.incoming_msgid, self.in_reply_to.outgoing_msgid)),
                    })
