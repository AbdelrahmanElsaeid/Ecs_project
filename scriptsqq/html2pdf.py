...
def usage():
    print(f'usage: {sys.argv[0]} -s static_root', file=sys.stderr)
    sys.exit(1)
...
def main():
    ...
    # Remove control characters, otherwise lxml will go kaboom.
    html = re.sub(r'[\x00-\x08\x0b\x0e-\x1f\x7f]', '', html)

    # Remove soft hyphens, otherwise weasyprint's line breaking code will
    # be confused and error out with an AssertionError.
    html = html.replace('\xad', '')

    # Replace unicode line separator character with a plain newline
    # character, otherwise weasyprint's line breaking code will be confused
    # and error out with an AssertionError.
    html = html.replace('\u2028', '\n')
